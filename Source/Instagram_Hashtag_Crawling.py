...
def get_urls(max_num_posts=100000):
    ...
    '''
    num_post = driver.find_element_by_xpath(
        '//*[@id="react-root"]/section/main/header/div[2]/div/div[2]/span/span').text
    ...
    while len(url_list) <= max_num_posts:
        ...
        for i in range(1, 9):
            post_line = driver.find_element_by_xpath(
...
def get_loc():
    ...
    게시물의 장소 정보를 반환한다.
    '''
    loc_list = driver.find_elements_by_xpath(
        '//*[@id="react-root"]/section/main/div/div/article/header/div[2]/div[2]/div[2]/a').text
...
# set parameter
chrome_driver_path = 'C:/Users/schwe/Pycharm/python/python_crawler_example_01/webdriver/chromedriver.exe'  # 크롬 드라이버 위치
# hashtag_list = ['감성카페', '분위기좋은카페', '서울카페', '예쁜카페', '카페', '카페그램', '카페스타그램','카페추천','카페탐방'] # 추출하고 싶은 게시물안에 속한 해시태그 리스트
hashtag_list = ['감성카페']  # 추출하고 싶은 게시물안에 속한 해시태그 리스트
ID = 'htht9494@naver.com'  # ID
PW = 'gkgudxo4117***'  # PW (github에 올릴때 반드시 가리고 올릴것!!)
